...
public class ArtilleryPlayer extends BasePlayer {
	...
	private MapLocation getBestFiringLocation() throws GameActionException{
		...
		if (enemyRobotsInRange.length != 0){
			...
			for (MapLocation firingLocation : enemyRobotLocations){
				if (3*numberOfAlliedUnitsHit(firingLocation) < 2*numberOfEnemyUnitsHit(firingLocation)){
	...
	private int numberOfEnemyUnitsHit(MapLocation firingLocation){
		return rc.senseNearbyGameObjects(Robot.class, firingLocation, GameConstants.ARTILLERY_SPLASH_RADIUS_SQUARED, this.myTeam.opponent()).length;
	}

	private int numberOfAlliedUnitsHit(MapLocation firingLocation){
		return rc.senseNearbyGameObjects(Robot.class, firingLocation, GameConstants.ARTILLERY_SPLASH_RADIUS_SQUARED, this.myTeam).length;
	}
